...
const VolvoBusBookingDetails = () => {
  ...
  const handleCancel = async (index) => {
    try {
      ...
      // Only remove the item from the frontend if the backend confirms successful deletion
      if (response?.payload?.success) {
        ...
        // Log the updated list in console
        console.log("Updated travel details:", travelDetails.filter((_, i) => i !== index));
      } else {
